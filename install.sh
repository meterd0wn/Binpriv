...
# Additions: Daniel Paul (aka @meterd0wn)
...
if [[ $(id -u) -ne 0 ]]; then
    ...
    if [[ ! "$(command -v binpriv)" ]]; then
        echo -e "\n${blueColour}[+] Copying the script binpriv.sh to the path /usr/local/bin/binpriv...\n${endColour}"
        sleep 2
        cp -v ./binpriv.sh /usr/local/bin/binpriv
    fi

    if [ -d "/usr/local/share/bins" ] && [ "$(command -v searchbins)" ] && [ "$(command -v binpriv)" ]; then
